...
pub fn get_liquidity_from_amounts(
    ...
    if sqrt_ratio_x64 <= sqrt_ratio_a_x64 {
        ...
        let liq_0 = get_liquidity_from_amount_0(sqrt_ratio_x64, sqrt_ratio_b_x64, amount_0)?;
        let liq_1 = get_liquidity_from_amount_1(sqrt_ratio_a_x64, sqrt_ratio_x64, amount_1)?;
        Ok(u128::min(liq_0, liq_1))
    } else {
...
pub fn get_delta_amount_0_signed(
    ...
    if liquidity < 0 {
        let liq_abs = u128::try_from(-liquidity)
            .map_err(|_| ErrorCode::CalculateOverflow)?;
        ...
        let liq_pos = u128::try_from(liquidity)
            .map_err(|_| ErrorCode::CalculateOverflow)?;
        get_delta_amount_0_unsigned(
            sqrt_ratio_a_x64,
            sqrt_ratio_b_x64,
